...
struct fixImageStatus fixImage(char* inputfilepath, char* outputfilepath, enum EDCMode form2EDCMode, bool verbose)
{
    ...
    if(form2EDCMode == INFER)
    {
		...
		int i;
        for(i = 0; i < sizeof(form2sectorsinbootloader) / sizeof(form2sectorsinbootloader[0]); ++i)
        {
	...
	int i;
    for(i = 0; i < filesize; i += SECTOR_SIZE)
    {
        // Read next sector.
        ...
        if(mode == MODE_0)
        {
			...
			int j;
            for(j = HEADER_OFFSET + HEADER_SIZE; j < SECTOR_SIZE; ++j)
            {
                if(sector[j] != 0x00)
                {
                    // Free memory.
            ...
            for(j = i + SECTOR_SIZE; j < filesize && remainder_is_zero; j += SECTOR_SIZE)
            {
                // Read next sector.
        ...
        else if(mode == MODE_2)
        {
            ...
            if(isForm2)
            {
                ...
                switch(form2EDCMode)
                {
                    ...
                    case COMPUTE:
                        ...
                        EDC = 0x00000000;
						int i;
                        for(i = CDROMXA_SUBHEADER_OFFSET; i < CDROMXA_FORM2_EDC_OFFSET; ++i)
                        {
                            EDC = EDC ^ sector[i];
            ...
            else
            {
				...
				int i;
                for(i = CDROMXA_SUBHEADER_OFFSET; i < CDROMXA_FORM1_EDC_OFFSET; ++i)
                {
                    EDC = EDC ^ sector[i];
                    EDC = (EDC >> 8) ^ EDCTable[EDC & 0x000000FF];
                }
